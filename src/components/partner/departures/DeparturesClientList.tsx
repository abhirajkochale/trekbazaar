                ...
                <thead className="bg-zinc-50/80 border-b border-zinc-200 sticky top-0 z-10 backdrop-blur-sm">
                  <tr>
                    <th className="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider cursor-pointer hover:bg-zinc-100 transition-colors" onClick={() => handleSort('departure_date')}>
                      <div className="flex items-center gap-2">Dates {sortConfig?.key === 'departure_date' && (sortConfig.direction === 'asc' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />)}</div>
                    </th>
                    <th className="px-6 py-4 text-xs font-bold text-zinc-500 uppercase tracking-wider cursor-pointer hover:bg-zinc-100 transition-colors" onClick={() => handleSort('trek_name')}>
                      <div className="flex items-center gap-2">Trek Name {sortConfig?.key === 'trek_name' && (sortConfig.direction === 'asc' ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />)}</div>
                    </th>
                ...
                <div key={dep.id} className="bg-white border border-zinc-200 rounded-2xl p-5 shadow-[0_2px_8px_rgba(0,0,0,0.04)] flex flex-col gap-4">
                  ...
                  <div className="bg-zinc-50 rounded-xl p-3 border border-zinc-100 flex flex-col gap-2">
                    <div className="flex items-center justify-between text-xs font-bold">
                      <span className="text-zinc-900 flex items-center gap-1.5"><Users className="w-4 h-4 .5 .5 text-zinc-400" /> {dep.booked_seats} Booked</span>
                      <span className="text-zinc-500">{dep.total_seats} Total</span>
                    </div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <Link href={`/partner/dashboard/departures/new?duplicate=${dep.id}`} className="flex-1 text-center text-xs font-bold text-zinc-700 bg-white border border-zinc-200 hover:bg-zinc-50 px-3 py-2 rounded-lg transition-colors flex items-center justify-center gap-1">
                      <Copy className="w-4 h-4 .5 .5" /> Duplicate
                    </Link>
                  </div>
                </div>
               );

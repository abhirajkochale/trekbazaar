  ...
  return (
    <div className="bg-white p-4 rounded-xl border border-zinc-200 shadow-sm mb-6 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-zinc-100">
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-500 mb-1">Verification</label>
          <select
            defaultValue={searchParams.get('verification') || 'all'}
            onChange={(e) => updateParams('verification', e.target.value)}
            className="block w-full py-1.5 pl-3 pr-8 border border-zinc-200 bg-white rounded-lg text-sm focus:outline-none focus:ring-1 focus:ring-tb-primary focus:border-tb-primary"
          >
        ...
        <div>
          ...
          >
            ...
            <option value="created_desc">Recently Joined</option>
